...
export async function runFuturesRunner(initialArgs: ModuleArgs): Promise<void> {
  ...
  const closeSniffer = () => {
    ...
    try {
      ...
      console.warn('[futures-runner] Error al cerrar el sniffer:', error);
    }
  ...
  const closeBrowser = async () => {
    ...
    try {
      ...
      console.warn('[futures-runner] Error al cerrar el navegador:', error);
    }
  ...
  const flush = async () => {
    ...
    try {
      ...
      console.warn('[futures-runner] Error durante flush:', error);
    }
  ...
  const shutdown = async (reason: EndReason, error?: Error) => {
    ...
    if (page) {
      try {
        ...
        console.warn('[futures-runner] Error al cerrar la página:', closeError);
      }
  ...
  const onMessage = (raw: unknown) => {
    try {
      ...
      console.error('[futures-runner] Mensaje inválido desde el padre:', error);
      return;
    }
  ...
  const handleUncaughtException = (error: unknown) => {
    const err = toError(error);
    console.error('[futures-runner] Excepción no controlada:', err);
    void shutdown('error', err);
  };

  const handleUnhandledRejection = (reason: unknown) => {
    const err = toError(reason);
    console.error('[futures-runner] Rechazo no controlado:', err);
    void shutdown('error', err);
  };
